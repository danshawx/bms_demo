...
LOG_MODULE_REGISTER(sensor_commands);
...
static uint8_t AccNameList[][SENSOR_NAME_MAX_LENGTH] = { "LIS2DW12"};
...
int Handle_Sensor_command(TMsg *Msg)
{
  ...
  /* Commands */
  LOG_DBG("subcmd is %d", Msg->Data[3]);
  switch (Msg->Data[3])
  {
...
static int SC_Set_Full_Scale(TMsg *Msg)
{
  ...
  {
    case SC_ACCELEROMETER:
    if (gsensor_set_attr(SENSOR_CHAN_ACCEL_XYZ, SENSOR_ATTR_FULL_SCALE, full_scale))
    {
      return 0;
    }
      // if (IsHybridAccSensor == HYBRID_SENSOR && AccInstance == IKS01A3_LIS2DTW12_0)
...
static int SC_Get_Full_Scale(TMsg *Msg)
{
  ...
  {
    case SC_ACCELEROMETER:
    if (gsensor_get_attr(SENSOR_CHAN_ACCEL_XYZ, SENSOR_ATTR_FULL_SCALE, &full_scale))
    {
      return 0;
    }
      // if (IsHybridAccSensor == HYBRID_SENSOR && AccInstance == IKS01A3_LIS2DTW12_0)
...
static int SC_Set_ODR(TMsg *Msg)
{
  ...
  {
    case SC_ACCELEROMETER:
    if (gsensor_set_attr(SENSOR_CHAN_ACCEL_XYZ, SENSOR_ATTR_SAMPLING_FREQUENCY, odr))
    {
      return 0;
    }
      // if (IsHybridAccSensor == HYBRID_SENSOR && AccInstance == IKS01A3_LIS2DTW12_0)
...
static int SC_Get_ODR(TMsg *Msg)
{
  ...
  {
    case SC_ACCELEROMETER:
    if (gsensor_get_attr(SENSOR_CHAN_ACCEL_XYZ, SENSOR_ATTR_SAMPLING_FREQUENCY, &odr))
    {
      return 0;
    }
      // if (IsHybridAccSensor == HYBRID_SENSOR && AccInstance == IKS01A3_LIS2DTW12_0)
