...
LOG_MODULE_REGISTER(nucleo_main);
...
int nucleo_main(void)
{
  ...
  */
  ...
  // for (;;)
  {
    ...
    if (AutoInit == 1U)
    {
      SensorsEnabled = ACCELEROMETER_SENSOR_ENABLED; //0xFFFFFFFF;
    }
...
static void Accelero_Sensor_Handler(TMsg *Msg, uint32_t Instance)
{
  ...
  struct sensor_axis_val hyb_acc;
  uint8_t status = 0;
  ...
  if (status == 1U)
  {
    ...
    LOG_DBG("acceleration(mg):x is %d y is %d, z is %d", acceleration.x, acceleration.y, acceleration.z);
    ...
    else if (AutoInit != 0U)
    {
      ...
      // (void)HAL_UART_Transmit(&UartHandle, (uint8_t *)DataOut, (uint16_t)strlen(DataOut), 5000);
      comm_uart_senddata((uint8_t *)DataOut, (uint16_t)strlen(DataOut));
    }
    else
    {
      /* Nothing to do */
